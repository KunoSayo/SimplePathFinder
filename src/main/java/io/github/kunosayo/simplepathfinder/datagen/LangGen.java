...
public class LangGen {
    ...
    public void initializeTranslations() {
        ...
        add("simple_path_finder.build.nav.batch_success", "成功构建导航区块，%s个更改，%s个失败，总计构建%s个，预期构建%s个", "Successfully built navigation chunk, dirty %s, failure %s, total %s, expected %s");
        ...
        add("gui.simple_path_finder.navigation.block_distance_config", "方块距离权重配置", "Block Distance Config");

        // 方块距离配置GUI
        add("gui.simple_path_finder.block_distance.title", "方块距离权重配置", "Block Distance Configuration");
